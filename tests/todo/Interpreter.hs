...
{-@ measure instrDenote @-}
{-@ instrDenote :: s:Stack -> i:{v:Instr | isbinOp v => len s >= 2 } 
                -> {zz:Maybe {v:Stack | len v = if (isbinOp i) then ((len s) - 1) else ((len s) + 1)} 
                   | zz = instrDenote s i }
  @-}
...
{-@ progDenote :: s:Stack -> is:{v:Prog | len s >= 2 * (binOps v)} -> Maybe Stack @-}
progDenote s [] = Just s
